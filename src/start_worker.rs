...
/// the worker is still running JS.
...
fn post_worker_init() -> Result<(), JsValue> {
    let worker = crate::mainthread::worker();
    let msg = Object::new();
    Reflect::set(
        &msg,
...
pub(crate) fn msg(e: &MessageEvent) -> bool {
    if is_worker_bootstrap_ready(e) {
        info!("Main: Worker bootstrap ready");
        if let Err(e) = post_worker_init() {
